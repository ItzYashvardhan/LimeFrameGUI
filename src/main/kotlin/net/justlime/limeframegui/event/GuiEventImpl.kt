 ...
 */
class GuiEventImpl(override var session: GuiSession, private val setting: GuiSetting) : GuiEventHandler {
    ...
    override fun onEvent(event: InventoryOpenEvent) {
        ...
        if (!hasTriggeredGlobalOpen.contains(player.name)) {
            ...
            setting.style.openSoundAlias?.let { alias ->
                val cleanAlias = alias.removePrefix("sound.")
                GuiSound.playPack(player, SoundRegistry.get(cleanAlias))
            }
        }
    }

    /**
     * Handles click events with a clear priority system.
     */
    override fun onEvent(event: InventoryClickEvent) {
        ...
        // Play Click Sound
        val aliasToPlay = event.item?.style?.clickSoundAlias ?: setting.style.clickSoundAlias
        aliasToPlay?.let { alias ->
            val cleanAlias = alias.removePrefix("sound.")
            GuiSound.playPack(player, SoundRegistry.get(cleanAlias))
        }
    }
    ...
    override fun onEvent(event: InventoryCloseEvent, plugin: JavaPlugin) {
        ...
        // Helps to Clean up the player's page tracking to prevent memory leaks.
        ...
        // This correctly plays the sound on non-Folia servers
